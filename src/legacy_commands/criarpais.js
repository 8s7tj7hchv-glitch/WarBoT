...
export default {
  ...
  async execute(i) { try { const c = countries.create({ guildId: i.guildId, guildName: i.guild?.name, leaderId: i.user.id, name: i.options.getString('nome'), code: i.options.getString('codigo'), emoji: i.options.getString('emoji') ?? '🏳️' }); await i.reply({ content: `✅ ${c.emoji} **${c.name}** (${c.code}) foi criado. Você é o líder.`, flags: MessageFlags.Ephemeral }); } catch (e) { await i.reply({ content: `❌ ${e.message}`, flags: MessageFlags.Ephemeral }); } }
};
