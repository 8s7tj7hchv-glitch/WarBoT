...
export default {
  ...
  async execute(i){
    ...
    if(!country)return i.reply({content:'❌ Você precisa pertencer a um país.',flags: MessageFlags.Ephemeral});
    ...
    await i.reply({embeds:[new EmbedBuilder().setTitle('🔬 Pesquisa & Tecnologia').setDescription(lines.join('\n').slice(0,4000)).addFields({name:'Pontos disponíveis',value:String(state.points),inline:true},{name:'Desbloqueadas',value:`${state.unlocked.length}/${tech.list().length}`,inline:true})],flags: MessageFlags.Ephemeral});
  }
};
