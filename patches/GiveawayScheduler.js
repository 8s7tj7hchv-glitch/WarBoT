import {awardProFromGiveaway} from '../pro/GiveawayProRewardService.js';
...
class GiveawayScheduler{
 ...
 async finish(id){
  ...
  const proAwards=await awardProFromGiveaway(g);
  ...
  try{
   ...
   await channel.send(g.winners.length
    ?`🎉 Parabéns ${g.winners.map(x=>`<@${x}>`).join(', ')}! Vocês venceram **${g.prize}**!${proAwards.length ? ` ⭐ Pro Player ativado automaticamente.` : ''}`
